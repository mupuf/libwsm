...
#define PREFIX                       "/etc/"
#define TMP_PREFIX                   ""
#define WAYLAND_POLICY_DIR           TMP_PREFIX"security/wsm/default"
#define WAYLAND_POLICY_PER_USER_DIR  TMP_PREFIX"security/wsm/per-user/default"
...
static int _init_policy_list(struct wsm_default_t *global)
{
	...
	struct dirent **namelist = NULL;
	int nb_users = scandir (WAYLAND_POLICY_PER_USER_DIR, &namelist, _filter_uid, alphasort);
	if (nb_users == -1 && errno != ENOENT) {
		DEBUG ("An error occurred when scanning user directories in '%s'.\n", WAYLAND_POLICY_PER_USER_DIR);
	}
	else
		DEBUG ("%d %s found.\n", nb_users, (nb_users!=1? "user directories":"user directory"));

	if (nb_users > 0) {	
		size_t pathlen = strlen (WAYLAND_POLICY_PER_USER_DIR);
		int i;
		for (i=0; i<nb_users; ++i)
		{
			struct dirent *ent = namelist[i];
			char *folder_path = malloc (pathlen + strlen(ent->d_name) + 2);
			sprintf (folder_path, "%s/%s", WAYLAND_POLICY_PER_USER_DIR, ent->d_name);
	...
	int nb_policies = scan_policy_folder (global, WAYLAND_POLICY_DIR, 0);
	if (nb_policies < 0)
		fprintf (stderr, "An error occurred when looking for policies in '%s'.\n", WAYLAND_POLICY_DIR);
	else
		total_policies += nb_policies;
...
void *ctor(void)
{
	void *config = malloc(sizeof(struct wsm_default_t));
	_init_policy_list(config);

	return config;
}

void dtor(void *config)
{
	_free_policy_list(config);
	free(config);
}
...
{
	void *config = malloc(sizeof(struct wsm_default_client_t));
